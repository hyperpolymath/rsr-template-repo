(** pseudocode_face.ml — Pseudocode syntax face for AffineScript
    Copyright (c) 2026 Jonathan D.A. Jewell (hyperpolymath)
    SPDX-License-Identifier: PMPL-1.0-or-later

    Transforms pseudocode-style syntax into canonical AffineScript AST.
...
(* Pseudocode keywords and mappings *)
let pseudocode_keywords = [
  ("FUNCTION", "def");
  ("RETURNS", "->");
  ("LET", "let");
  ("IF", "if");
...
(* Transform pseudocode tokens to AffineScript tokens *)
let transform_token (tok : Token.t) : Token.t =
  let open Token in
  match tok with
  | Ident s when List.assoc_opt s pseudocode_keywords <> None ->
      Ident (List.assoc s pseudocode_keywords)
  | _ -> tok

(* Transform pseudocode AST to AffineScript AST *)
let transform_pseudocode (prog : Ast.program) : Ast.program =
  (* Apply token transformation to the entire AST *)
  let rec transform_expr (e : Ast.expr) : Ast.expr =
    match e with
    | EIdent (loc, s) ->
        let s' = match List.assoc_opt s pseudocode_keywords with
...
(* Entry point for the pseudocode face *)
let transform_program (prog : Ast.program) : Ast.program =
  transform_pseudocode prog
