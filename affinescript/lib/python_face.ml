(** python_face.ml — Python syntax face for AffineScript
...
(* Transform Python tokens to AffineScript tokens *)
let transform_token (tok : Token.t) : Token.t =
  let open Token in
  match tok with
  | Ident s when List.assoc_opt s python_keywords <> None ->
      Ident (List.assoc s python_keywords)
  | _ -> tok

(* Transform Python AST to AffineScript AST *)
let transform_python (prog : Ast.program) : Ast.program =
  (* Apply token transformation to the entire AST *)
  let rec transform_expr (e : Ast.expr) : Ast.expr =
    match e with
    | EIdent (loc, s) ->
        let s' = match List.assoc_opt s python_keywords with
...
(* Entry point for the Python face *)
let transform_program (prog : Ast.program) : Ast.program =
  transform_python prog
