...
bash "$root/scripts/validate-session-contracts.sh" --typecheck "$fixture/plain.ncl" "$fixture/wrapped.k9.ncl"
if bash "$root/scripts/validate-session-contracts.sh" --typecheck "$fixture/bad.k9.ncl"; then
    echo 'Invalid Nickel was accepted' >&2
    exit 1
fi
echo 'PASS: plain and wrapped Nickel accepted; malformed Nickel rejected'
