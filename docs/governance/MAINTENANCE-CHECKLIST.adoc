...
- `machine-readable/policies/SOFTWARE-DEVELOPMENT-APPROACH.a2ml` (machine-readable)
...
- [ ] Keep ownership/governance files present:
  - `MAINTAINER` in root
  - `.github/CODEOWNERS`
- [ ] Keep machine-readable canonical structure under `machine-readable/`:
...
- [ ] Keep security metadata present:
  - `www/.well-known/security.txt` and relevant policy metadata
...
- [ ] Maintain `www/.well-known/` for public metadata where applicable (`security.txt`, `humans.txt`, `ads.txt` mirrors if used); a root `.well-known/` is legacy (issue #53).
...
Optional directive write-back to repos that already have `machine-readable/`:
