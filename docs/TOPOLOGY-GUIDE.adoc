...
[source,bash]
----
cp ~/Documents/hyperpolymath-repos/rsr-template-repo/TOPOLOGY.md ./TOPOLOGY.md
...
for repo in ~/Documents/hyperpolymath-repos/*/; do
...
- **STATE.scm** — machine-readable state (tasks, blockers, next actions)
- **ECOSYSTEM.scm** — position in the wider project ecosystem
- **META.scm** — architecture decisions and design rationale
- **0-AI-MANIFEST.a2ml** — AI agent entry point and invariants

TOPOLOGY.md is the _visual summary_ for humans; the SCM files are the
